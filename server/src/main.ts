...
async function start(server: express.Express, app: NestExpressApplication, https_port: number, httpsOptions, http_port: number = 86, internalHttpsOptions, https_internal_port: number = 664) {
  try {
    ...
    // }
    const httpsInternalServer = undefined; //  !!internalHttpsOptions ? https.// createServer(internalHttpsOptions, applicationInstance) : // undefined;
...
async function bootstrap() {
  ...
  const internalHttpsOptions: HttpsOptions = undefined;
  //  (!!process.env.internal_pfx || !!process.env.internal_key) ? {
